...
STRATEGIES += [
    ...
    ['yt-dlp', '--ignore-errors', '--extractor-args', 'youtube:player_client=android,mweb',
    ...
    ['yt-dlp', '--ignore-errors', '--extractor-args', 'youtube:player_client=ios,web',
     '-f', fmt_str, '--format-sort', f'res:{height},fps', '--merge-output-format', 'mp4',
     '--no-playlist', '--no-check-certificates'] + sub_args + cookie_args + ['-o', '/tmp/video.%(ext)s', url],

    ['yt-dlp', '--ignore-errors', '-f', 'bestvideo+bestaudio/best', '--merge-output-format', 'mp4',
     '--no-playlist', '--no-check-certificates'] + sub_args + cookie_args + ['-o', '/tmp/video.%(ext)s', url],

    ['yt-dlp', '--ignore-errors', '-f', f'best[height<={height}]/best', '--merge-output-format', 'mp4',
     '--no-playlist', '--no-check-certificates'] + sub_args + cookie_args + ['-o', '/tmp/video.%(ext)s', url]
]
...
if callback_url and tg_file_ids:
    ...
    cb_data = {
        ...
        'media_id': media_id,
        'cloud_file_id': tg_file_ids[0],
        'file_id': tg_file_ids[0],
        'thumbnail_file_id': cover_thumb_id or tg_file_ids[0],
        'thumbnail_url': f'/api/thumbnail/{cover_thumb_id or tg_file_ids[0]}',
        'duration': duration,
        'size': final_size,
        'category': category,
        'message': f'✅ [{quality}] Berhasil diunduh dan diunggah ke Cloud Telegram ({final_size/(1024*1024):.1f} MB)',
        'media': {
            'sourceUrl': url,
            'title': title,
